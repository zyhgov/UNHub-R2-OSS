  ...
  >
    ...
    <div class="app-bar">
      <a class="app-title-container" style="display: flex; align-items: center;" href="/">
        <img src="/assets/homescreen.png" alt="UNHub OSS" style="height: 40px;margin-left: 5px;" />
        <h1 class="app-title" style="font-size: 20px;margin: 0 25px 0 8px; user-select: none;">UNHub OSS</h1>
      </a>

      <input type="search" v-model="search" aria-label="Search" placeholder="键入关键词以全局搜索文件" />
      <div class="menu-button">
        <button class="circle" @click="showMenu = true" style="display: flex; align-items: center;background-color: rgb(245, 245, 245);">
          <p style="
              ...
              margin: 0 10px 0 0;
              font-size: 16px;
              font-family: 'OpenAISans', -apple-system, BlinkMacSystemFont, 'Segoe UI Adjusted',
...
export default {
  data: () => ({
    ...
    uploadProgress: null,
    uploadQueue: [],
    backgroundImageUrl: "/assets/2.jpg"
  }),
    ...
    cwd: {
      handler() {
        ...
        document.title = this.cwd.replace(/.*\/(?!$)|\//g, "") === "/" 
            ? "UNHub R2 OSS"
            :`${this.cwd.replace(/.*\/(?!$)|\//g, "") || "/" } - UNHub R2 OSS`;
      },
      immediate: true,
    },
  },
